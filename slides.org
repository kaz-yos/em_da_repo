...
[[./source/em_algo.png]]
\footnotesize
(To be) Presented as a part of the Mini Statistics Camp 2019 hosted by the [[http://bioinformatics.bwh.harvard.edu][BWH/HMS Bioinformatics Club]]
...
\newpage
- Now we have the probabilities of coin identities (soft assignment cite:hastieElementsStatisticalLearning2016).
\footnotesize
| Index | Coin    |              Prob. Coin A |          Prob. Coin B |   Heads | Heads Coin A                     | Heads Coin B                 |
|   $i$ | $Z_{i}$ | $E[(1-Z_{i})\vert X_{i}]$ | $E[Z_{i}\vert X_{i}]$ | $X_{i}$ | $E[(1-Z_{i}) X_{i} \vert X_{i}]$ | $E[Z_{i} X_{i} \vert X_{i}]$ |
|-------+---------+---------------------------+-----------------------+---------+----------------------------------+------------------------------|
...
ip_iter2_out <- ip_iter2(c(0.6,0.5), c(1,1), c(1,1), iter = 10) %>%
  ...
  if (i > 1) {
    for (j in seq_len(i-1)) {
      ...
      segments3d(x = c(ip_iter2_out$theta0[j], ip_iter2_out$theta0[j + 1]),
                 ...
                 z = c(exp(llik_incompl(ip_iter2_out$theta0[j], ip_iter2_out$theta1[j])),
                       exp(llik_incompl(ip_iter2_out$theta0[j + 1], ip_iter2_out$theta1[j + 1]))) / max_incompl_lik)
    }
  }
  ## Add gt
  surface3d(x = grid, y = grid, z = ip_iter2_out$mat[[i]] / max(ip_iter2_out$mat[[i]]),
            back = "line", front = "line", col = "blue",
            lwd = 1, alpha = 0.1)

  ## Save E Step plot
  rgl.snapshot(filename = sprintf("./source/ip_rgl_%s_e.png", i-0),
               fmt = "png", top = TRUE)
  ## Add segment indicating move
  segments3d(x = c(ip_iter2_out$theta0[i], ip_iter2_out$theta0[i + 1]),
             y = c(ip_iter2_out$theta1[i], ip_iter2_out$theta1[i + 1]),
             z = c(exp(llik_incompl(ip_iter2_out$theta0[i], ip_iter2_out$theta1[i])),
                   exp(llik_incompl(ip_iter2_out$theta0[i + 1], ip_iter2_out$theta1[i + 1]))) / max_incompl_lik)
  ## Save M Step plot
  rgl.snapshot(filename = sprintf("./source/ip_rgl_%s_m.png", i-0),
               fmt = "png", top = TRUE)
}
...
** DA: TODOs
- Handling of bimodality
- Pointers to theory of convergence to posterior
...
** Stan: TODOs
- Check convergence diagnostics (here $\Rhat$ looks ok) and meaning (same marginal means for both coins)
- Add constrained prior version
