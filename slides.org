...
- This is very similar to the EM algorithm, which uses the marginalized likelihood.
...
*** @@latex:@@                                                        :BMCOL:
    ...
    :BEAMER_col: 0.50
    :END:
- Some initial draws from the first chain
- More easily transition between two modes?
...
path_coin_sample <-
  bind_cols(array_coin_sample[seq_len(max_iter), , c("theta[1]")] %>%
            as_tibble() %>%
            gather(key = chain, value = theta0) %>%
            mutate(chain = gsub("chain:","", chain)),
            array_coin_sample[seq_len(max_iter), , c("theta[2]")] %>%
            as_tibble() %>%
            gather(key = chain, value = theta1) %>%
            mutate(chain = gsub("chain:","", chain)) %>%
            select(-chain)) %>%
  group_by(chain) %>%
  ## First two are duplicated.
  mutate(iter = c(-1,0,seq_len(n() - 2))) %>%
  filter(iter >= 0)
...
coin_sample_ordered <- rstan::stan(model_code = stan_code_ordered,
                                   ...
                                   iter = 10^4,
                                   chains = 12)
out <- rstan::extract(coin_sample_ordered, permuted = FALSE, inc_warmup = TRUE)
dim(out)
dimnames(out)
#+END_SRC
\normalsize
...
*** @@latex:@@                                                        :BMCOL:
    ...
    :BEAMER_col: 0.50
    :END:
- (TODO) We have a unimodal posterior as we effectively restricted our parameter space to be the upper diagonal.
