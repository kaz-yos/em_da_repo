...
\maketitle
\section{Opening}
\label{sec:org5c1c0c9}


\section{Introduction}
\label{sec:orga8d7cc7}
\begin{frame}[label={sec:org4a0e325}]{Article Covered}
\begin{itemize}
...
\end{center}
\footnotesize
To be presented as a part of the Mini Statistics Camp 2019 hosted by the \href{http://bioinformatics.bwh.harvard.edu}{BWH/HMS Bioinformatics Club}. In addition to the \hyperlink{sec:org1a37733}{EM Algorithm}, the slides contain the corresponding Bayesian inference using the \hyperlink{sec:org33f358f}{Data Augmentation} method and \hyperlink{sec:org384cd8f}{Stan}.
\end{frame}

\begin{frame}[label={sec:org878f064}]{Motivation}
\begin{itemize}
...
\begin{frame}[label={sec:org7f7d32f}]{Incomplete data}
\begin{columns}
...
\begin{frame}[label={sec:org853d4b1}]{EM Algorithm Applications}
\begin{itemize}
...
\label{sec:orgb51202c}
\begin{frame}[label={sec:org482fcb3}]{A Coin-Flipping Experiment}
\begin{itemize}
...
\begin{frame}[label={sec:orgbfe5ac6}]{A Coin-Flipping Experiment (More Formal)}
\begin{itemize}
\item Two coins \(A\) (0) and \(B\) (1) with unknown head probabilities \((\theta_{0},\theta_{1})\)
\item For \(i = 1, \dots, 5\)
\begin{enumerate}
...
\label{sec:org3850f77}
\begin{frame}[label={sec:orgfbe448d}]{Complete-Data Maximum Likelihood}
\begin{itemize}
...
\begin{frame}[label={sec:org10bb152}]{Complete-Data Likelihood Visualization}
\begin{columns}
...
\item See the \hyperlink{eq:orgf2d671c}{likelihood expression}.
\end{itemize}
\end{column}
...
\label{sec:orgf97186c}
\begin{frame}[label={sec:org04300ce}]{A Contrived Coin-Flipping Experiment}
\begin{itemize}
...
\begin{frame}[label={sec:org54e0684}]{A Contrived Coin-Flipping Experiment (More Formal)}
\begin{itemize}
...
\begin{frame}[label={sec:org727f254}]{How Do We Approach Incomplete Data}
\begin{itemize}
...
\end{itemize}
\item The \hyperlink{sec:org1a37733}{Expectation-Maximization (EM) Algorithm} \cite{dempsterMaximumLikelihoodIncomplete1977} is a refinement of this idea for MLE.
\item The \hyperlink{sec:org33f358f}{Data Augmentation Method} \cite{tannerCalculationPosteriorDistributions1987} is another type of refinement for Bayesian estimation.
\end{itemize}
\end{frame}

\section{EM Algorithm}
\label{sec:org1a37733}
\begin{frame}[label={sec:orgfefa977}]{}
\begin{center}
\resizebox{\linewidth}{!}{Expectation-Maximization Algorithm}
\end{center}
\end{frame}

\begin{frame}[label={sec:orgcaa3086}]{EM Algorithm to the Rescue}
\begin{itemize}
...
\begin{frame}[label={sec:org2b336bc}]{}
\begin{center}
...
\begin{frame}[label={sec:org139b6e6}]{Parameter Initialization}
\begin{itemize}
...
\normalsize
\begin{itemize}
\item See also \hyperlink{sec:org410780b}{Coin Probability Derivation}.
\end{itemize}

\newpage
\begin{itemize}
\item Now we have the probabilities of coin identities.
\item This is sometimes called "soft assignment" \cite{hastieElementsStatisticalLearning2016}.
\end{itemize}
\footnotesize
\begin{center}
\begin{tabular}{r|l|rr|r|ll|}
...
\item These maximize the \hyperlink{eq:org3a4a0d9}{expected log likelihood}.
\end{itemize}
\end{frame}
...
\begin{frame}[label={sec:orgbe2b2f5},fragile]{EM Step (2)}
 \scriptsize
\begin{minted}[frame=lines,linenos=false]{r}
...
\begin{frame}[label={sec:orgd6466f6}]{Visual Representation of Iteration}
\begin{columns}
\begin{column}{0.50\columnwidth}
\begin{itemize}
...
\begin{frame}[label={sec:org2d1b20a}]{Multiple Initialization and Label Indeterminancy}
\begin{columns}
\begin{column}{0.50\columnwidth}
\begin{itemize}
...
\begin{frame}[label={sec:org5ef80b1}]{Incomplete-Data Likelihood}
\begin{columns}
\begin{column}{0.50\columnwidth}
\begin{itemize}
\item In this specific instance, the \hyperlink{eq:org0036dd7}{incomplete-data likelihood} can be graphed with grid search as the parameter space is small and low dimensional ([0,1]\textsuperscript{2}).
...
\begin{frame}[label={sec:orgd3dd957}]{EM: Incomplete-Data Likelihood}
\begin{columns}
\begin{column}{0.50\columnwidth}
\begin{itemize}
...
\begin{frame}[label={sec:org56259c5}]{EM: E Step (0)}
\begin{columns}
\begin{column}{0.50\columnwidth}
\begin{itemize}
...
\begin{frame}[label={sec:org7295130}]{EM: M Step (0)}
\begin{columns}
\begin{column}{0.50\columnwidth}
\begin{itemize}
...
\begin{frame}[label={sec:orgd4e2a2d}]{EM: E Step (1)}
\begin{columns}
\begin{column}{0.50\columnwidth}
\begin{itemize}
...
\begin{frame}[label={sec:org2ed4f6e}]{EM: M Step (1)}
\begin{columns}
\begin{column}{0.50\columnwidth}
\begin{itemize}
...
\begin{frame}[label={sec:orgcbdd0cf}]{EM: E Step (2)}
\begin{columns}
\begin{column}{0.50\columnwidth}
\begin{itemize}
...
\begin{frame}[label={sec:org4f0d978}]{EM: M Step (2)}
\begin{columns}
\begin{column}{0.50\columnwidth}
\begin{itemize}
...
\begin{frame}[label={sec:org2c65f85}]{EM: E Step (3)}
\begin{columns}
\begin{column}{0.50\columnwidth}
\begin{itemize}
...
\begin{frame}[label={sec:org2ed33e5}]{EM: M Step (3)}
\begin{columns}
\begin{column}{0.50\columnwidth}
\begin{itemize}
...
\begin{frame}[label={sec:orga382905}]{EM: E Step (4)}
\begin{columns}
\begin{column}{0.50\columnwidth}
\begin{itemize}
...
\begin{frame}[label={sec:orgc542fbc}]{EM: M Step (4)}
\begin{columns}
\begin{column}{0.50\columnwidth}
\begin{itemize}
...
\begin{frame}[label={sec:orga103cf5}]{EM: E Step (5)}
\begin{columns}
\begin{column}{0.50\columnwidth}
\begin{itemize}
...
\begin{frame}[label={sec:org850e7e8}]{EM: M Step (5)}
\begin{columns}
\begin{column}{0.50\columnwidth}
\begin{itemize}
...
\label{sec:org92988d1}
\appendix
\begin{frame}[label={sec:orge15ef0e}]{}
\begin{center}
...
\begin{frame}[label={sec:org410780b}]{Coin Probability Derivation}
\begin{itemize}
...
\item Therefore, the \hyperlink{sec:org1a37733}{EM algorithm} is equivalent to the update rule with the guaranteed improvement at each step.
\end{itemize}
\end{frame}

\section{DA Method}
\label{sec:org33f358f}
\appendix
\begin{frame}[label={sec:orgb6eddda}]{}
\begin{center}
\resizebox{\linewidth}{!}{Data Augmentation Method}
\end{center}
\end{frame}

\begin{frame}[label={sec:orgc9601e8}]{From EM to DA}
\begin{itemize}
...
\begin{frame}[label={sec:orgf8d534c}]{Model Configuration}
\begin{itemize}
...
\begin{frame}[label={sec:orgcef56e4}]{Parameter Initialization}
\begin{itemize}
\item Randomly initialize the parameters
\begin{itemize}
\item \(\theta_{0}^{(0)} := 0.6\)
...
\begin{frame}[label={sec:orgf5b3a26}]{Visual Representation of Initial Iterations}
\begin{columns}
\begin{column}{0.50\columnwidth}
\begin{itemize}
...
\begin{frame}[label={sec:orgf90fe28}]{Visual Representation of Posterior Samples}
\begin{columns}
\begin{column}{0.50\columnwidth}
\begin{itemize}
...
\begin{frame}[label={sec:orgf7ac0d4}]{Visual Representation of Posterior Density}
\begin{columns}
\begin{column}{0.50\columnwidth}
\begin{itemize}
\item The first 10\% of posterior samples were discarded to reduce the influence of the initialization values.
...
\begin{frame}[label={sec:org230de09}]{DA: Animation}
\begin{columns}
\begin{column}{0.50\columnwidth}
\begin{itemize}
\item Incomplete-data likelihood function.
...
\section{Stan}
\label{sec:org384cd8f}
\appendix
\begin{frame}[label={sec:org85b7b00}]{}
\begin{center}
\resizebox{\linewidth}{!}{Sampling Using Stan}
\end{center}
\end{frame}

\begin{frame}[label={sec:orgced918b}]{Stan: Hamiltonian Monte Carlo}
\begin{itemize}
...
\begin{frame}[label={sec:orgaae53a8}]{Stan Diagnostic Plots}
\begin{columns}
\begin{column}{0.50\columnwidth}
\begin{center}
...
\begin{frame}[label={sec:orgde15bd8}]{Visual Representation of Stan Initial Iterations}
\begin{columns}
\begin{column}{0.50\columnwidth}
\begin{itemize}
...
\begin{frame}[label={sec:orge70d5ac}]{Stan Posterior Samples}
\begin{columns}
\begin{column}{0.50\columnwidth}
\begin{itemize}
...
\begin{frame}[label={sec:org520de6c}]{Stan Diagnostic Plots (Ordered Prior)}
\begin{columns}
\begin{column}{0.50\columnwidth}
\begin{center}
...
\begin{frame}[label={sec:org08a299e}]{Visual Representation of Stan Initial Iterations (Ordered Prior)}
\begin{columns}
\begin{column}{0.50\columnwidth}
\begin{itemize}
...
\begin{frame}[label={sec:org689af8c}]{Stan Posterior Samples (Ordered Prior)}
\begin{columns}
\begin{column}{0.50\columnwidth}
\begin{itemize}
